...
mcp/                      `@lovinka/deployik-mcp` — MCP server that wraps the Deployik API for AI use
  package.json            Published to npm; `bin: deployik-mcp` consumed via `npx -y @lovinka/deployik-mcp`
  scripts/copy-recipes.mjs  Prebuild: copies .claude/skills/deployik-howto/*.md into src/knowledge/recipes/ and emits a recipes.generated.ts module so the published bundle ships the docs inline
  src/
    index.ts              Stdio entry: loadEnv → buildServer → StdioServerTransport
    server.ts             buildServer(): wires DeployikClient + registers all tools + prompts
    client/
      http.ts             DeployikClient: fetch wrapper with Bearer auth, timeout, structured ApiError
      types.ts            TS types mirrored from web/src/types/api.ts (manual sync — small surface)
      errors.ts           ApiError + status→hint map (401/403/404/409/429/5xx)
    config/
      env.ts              Reads DEPLOYIK_URL, DEPLOYIK_TOKEN (+ .deployik/token fallback)
      binding.ts          Read/write .deployik/binding.json + auto-append .deployik/ to consuming repo .gitignore
      cache.ts            Read/write .deployik/cache.json (TTL'd project + workspace list)
      audit.ts            Append-only .deployik/audit.log of destructive calls; redacts value/secret/password keys
    resolve/project.ts    Identity resolution chain: project_id → slug → binding → git-remote → single-project workspace
    knowledge/
      index.ts            Loads recipes.generated.ts, splits skill .md by `## <slug>` headers into typed RecipeTopics
      prompts.ts          Registers one MCP prompt per recipe topic (deployik_recipe_*)
      recipes.generated.ts  AUTO-GENERATED by scripts/copy-recipes.mjs; do not edit
    tools/
      _helpers.ts         Shared registerTool wrapper: ToolDef → SDK registerTool + audit logging + structured errors
      projects.ts         list / get / create / update / delete (tiered safety on delete)
      deployments.ts      list / get / trigger / get_deploy_logs / get_deploy_screenshot
      env.ts              list / upsert / bulk_set / delete (env vars; bulk_set is destructive)
      secrets.ts          list / upsert / bulk_set / delete (secrets; same shape as env)
      domains.ts          list / add / verify / update (move + set_primary) / delete
      auto_build.ts       get / configure / delete
      protection.ts       get / set / regenerate (per-environment passwords)
      volumes.ts          list / delete / recreate (per env)
      github.ts           list_repos / list_branches / inspect_repo (monorepo detection)
      workspaces.ts       list_workspaces / get_platform_info / whoami / get_health
      analytics.ts        get_project_analytics / verify_project_analytics
      email.ts            get / update / test_smtp (Webglobe SMTP + reCAPTCHA)
      tokens.ts           list_my_tokens / create_my_token / revoke_my_token
      help.ts             list_recipes / get_recipe (bundled knowledge surface)
      workflows.ts        Composite tools: init_in_repo, setup_project_from_repo, deploy_project (wait+confirm), set_secret, set_env_var, wait_for_deployment, tail_latest_logs, debug_failed_deployment, get_project_health, show_binding
      index.ts            registerAllTools(server, ctx)
    lib/
      safety.ts           Tiered safety: read | mutating | destructive | destructive_production; returns dry-run preview when unconfirmed
      poll.ts             pollUntil(probe, predicate, opts) + sleep
      logs.ts             formatLogs(): stderr prefix, error-line anchoring (↑), byte/line budget, truncation hint
      format.ts           Project/deployment/domain/volume/protection text renderers + renderDryRun
  .deployik/              (written by init_in_repo at the consuming repo root, NOT inside this repo)
    binding.json          { project, workspace, defaultEnvironment }
    cache.json            TTL'd project + workspace list (1h)
    token                 (optional, mode 0600) fallback when DEPLOYIK_TOKEN env var unset
    audit.log             Line-delimited JSON of every destructive call (secret values redacted)
```
